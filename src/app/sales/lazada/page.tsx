...
// Assuming these are the providers for Lazada
...
export default function LazadaHubPage() {
            ...
            <main className="flex min-h-svh flex-1 flex-col gap-4 bg-muted/40 p-4 md:p-10">
                <div className="flex items-center gap-4">
                    ...
                    <h1 className="text-lg md:text-xl font-bold font-headline text-foreground">
                        {t.sales.lazada} - {t.shipping.receiptPage.title}
                    </h1>
                </div>

                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                    {shippingProviders.map(provider => (
                        <Link href={`/sales/lazada/${provider.name.toLowerCase()}`} key={provider.name}>
