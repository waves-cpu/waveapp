...
export async function addBulkProducts(products: any[]) {
    ...
    `);
    const updateProductStmt = db.prepare('UPDATE products SET name = ?, category = ?, imageUrl = ? WHERE id = ?');
    
    const findVariantBySkuStmt = db.prepare('SELECT id, stock FROM variants WHERE sku = ?');
    const insertVariantStmt = db.prepare('INSERT INTO variants (productId, name, sku, price, stock) VALUES (?, ?, ?, ?, ?)');
    const updateVariantStmt = db.prepare('UPDATE variants SET stock = ?, price = ?, name = ? WHERE id = ?');
        ...
        products.forEach(productData => {
            ...
            if (productRecord) {
                ...
                if (productData.name && productData.category) {
                     updateProductStmt.run(productData.name, productData.category, productData.imageUrl || 'https://placehold.co/100x100.png', productId);
                }
            } else {
            ...
            productData.variants.forEach((variant: any) => {
                ...
                if (existingVariant) {
                    ...
                    updateVariantStmt.run(newStockLevel, variant.price, variant.name, variantId);
                } else {
                    // Insert new variant
                    const variantResult = insertVariantStmt.run(productId, variant.name, variant.sku, variant.price, variant.stock);
